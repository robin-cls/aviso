import os
import pytest
from aviso_client.catalog_parser.catalog_client import fetch_catalog, get_details, _request_catalog, _request_product, _get_product_from_title, search_granules
from aviso_client.catalog_parser.models import AvisoCatalog

@pytest.fixture(autouse=True)
def mock_post(mocker, catalog_response):
    mock_post = mocker.patch('aviso_client.catalog_parser.catalog_client.requests.post')
...
def mock_get(mocker, product_response):
    mock_get = mocker.patch('aviso_client.catalog_parser.catalog_client.requests.get')
    mock_response = mocker.MagicMock()
    mock_response.status_code = 200
...
def test_request_catalog(mock_post):
    ...
    expected_url = os.path.join("https://sextant.ifremer.fr/geonetwork/srv/api/", 'search/records/_search')
...
def test_request_product(mock_get):
    ...
    assert call_args[0] == f"https://sextant.ifremer.fr/geonetwork/srv/api/records/{_id}"
...
@pytest.mark.parametrize('title, id', [('Sample Product 1', 'product1'), ('Sample Product 2', 'product2')])
