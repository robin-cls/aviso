import pytest
from aviso_client.catalog_client.geonetwork.query_builder import GeoNetworkQueryBuilder, InvalidFieldError
...
def test_should_match_clause():
    ...
    assert payload["query"]["bool"]["should"][0] == {"match": {"platforms": "Sentinel-3"}}


def test_filter_range_clause():
    builder = GeoNetworkQueryBuilder()
    payload = builder.filter_range("date", {"gte": "2020-01-01", "lte": "2022-01-01"}).build()
    
    assert "filter" in payload["query"]["bool"]
    assert payload["query"]["bool"]["filter"][0] == {
        "range": {
            "date": {"gte": "2020-01-01", "lte": "2022-01-01"}
        }
    }
...
def test_combined_clauses():
    ...
    payload = (
        ...
        .must_not_term("_id", "123")
        .should_term("platforms", "Sentinel-3")
        .filter_term("th_odatis_centre_donnees.default", "CDS-AVISO")
        .build()
    )
...
def test_invalid_operator_raises_error():
    ...
    with pytest.raises(ValueError):
        builder._add_clause("must", "unsupported_operator", "platforms", "test")
