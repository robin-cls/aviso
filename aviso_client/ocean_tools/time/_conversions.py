...
def julian_day_to_numpy(
        ...
        reference: np.datetime64 = CNES_DATE_REFERENCE) -> np.datetime64:
    ...
    """
    return (reference + np.timedelta64(julian_day[0], 'D') +
...
def fractional_julian_day_to_numpy(
    ...
    return julian_day_to_numpy((int(day), int(hour), hour_fraction * 3600),
                               reference=reference)


def numpy_to_julian_day(
        timestamp: np.datetime64,
        reference: np.datetime64 = CNES_DATE_REFERENCE
) -> tuple[int, int, float]:
    ...
    seconds = (delta - np.timedelta64(days, 'D') -
               np.timedelta64(hours, 'h')) / np.timedelta64(1, 's')
    return days, hours, seconds


def numpy_to_fractional_julian_day(
        timestamp: np.datetime64,
        reference: np.datetime64 = CNES_DATE_REFERENCE) -> float:
