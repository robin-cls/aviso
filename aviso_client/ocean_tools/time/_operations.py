...
def periods_envelop(periods: Iterable[Period]) -> Period:
    ...
    :
        The Period envelop
    """
    return reduce(_periods_envelop, periods)


def _periods_envelop(period1: Period, period2: Period) -> Period:

    if period1.start == period2.start:
        include_start = period1.include_start | period2.include_start
        start = period1.start
    elif period1.start < period2.start:
        include_start = period1.include_start
        start = period1.start
    elif period1.start > period2.start:
        include_start = period2.include_start
        start = period2.start

    if period1.stop == period2.stop:
        include_stop = period1.include_stop | period2.include_stop
        stop = period1.stop
    elif period1.stop > period2.stop:
        include_stop = period1.include_stop
        stop = period1.stop
    elif period1.stop < period2.stop:
        include_stop = period2.include_stop
        stop = period2.stop
